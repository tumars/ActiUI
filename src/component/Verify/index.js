    ...
    render() {
        ...
        return (
            <div className={style.content}>
                <input type="tel" ref="PhoneInput" onFocus={() => this.cleartip()} className={style.mobileinput} name="mobile" maxLength="11" placeholder="请输入手机号码" />
                <div className={style.validbox} id="Codes">
                    <section>
                        <input type="text" ref="ValidInput" onFocus={() => this.cleartip()} maxLength="4" placeholder="输入验证码" />
                    </section>
                    <section>
                        {this.state.isTiming ? (<span>{this.state.time}</span>) : (<span onClick={() => this.sendClick()}>获取</span>)}
                    </section>
                </div>
                <div className={style.tip}>{this.state.tip}{verifyTip}</div>
                <span className={style.btn} onClick={() => this.confirmClick()}>确认</span>
            </div>
        )
    }
}
